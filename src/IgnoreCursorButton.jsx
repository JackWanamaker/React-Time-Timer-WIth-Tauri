...
function handleSymbol(ignoreCursor) {
    if (ignoreCursor) {
        return returnNormal();
    }
    else {
...
const IgnoreCursorButton = ({ignoreCursor, setIgnoreCursor}) => {

async function switchIgnoreState() {
    const appWindow = await getCurrentWindow();
    await appWindow.setIgnoreCursorEvents(!ignoreCursor);
    setIgnoreCursor(!ignoreCursor);
}
    
    return (
        <button onClick={switchIgnoreState} className="settings-button">
            {handleSymbol(ignoreCursor)}
        </button>
    )
}

export default IgnoreCursorButton